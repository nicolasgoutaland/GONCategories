...
- (NSMutableString *)ucFirst
{
	...
	[self replaceCharactersInRange:NSMakeRange(0, 1)
...
{
    static NSDictionary *dicReplaceRules = nil;
    ...
    // Update string
    for (NSString *key in dicReplaceRules)
    {
        [self replaceOccurrencesOfString:key
                              withString:[dicReplaceRules objectForKey:key]
                                 options:0
                                   range:self.range];
    }
...
#warning Evol : Use regex to extract HTML entities ?
    ...
    {
        [self replaceOccurrencesOfString:htmlEntity
                              withString:[dicHTMLEntitiesInverted objectForKey:htmlEntity]
                                 options:0
                                   range:self.range];
    }
...
static NSDictionary *dicHTMLEntitiesInverted;
+ (void)load
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        ...
        dicHTMLEntitiesInverted = [dicHTMLEntities invertedDictionary];
    });
}
